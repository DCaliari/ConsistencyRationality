...
XYZW_SEQ = xlsread('DATI.xlsx','Variables','EI1:EI146');
WZYX_SEQ = xlsread('DATI.xlsx','Variables','EH1:EH146');

ABCD_SEQ = xlsread('DATI.xlsx','Variables','EG1:EG146');
DCBA_SEQ = xlsread('DATI.xlsx','Variables','EF1:EF146');
...
bin(:,1) = ( XYZW_SEQ==1 & deliberate_time==0 ) ;
bin(:,2) = (  WZYX_SEQ==1 & deliberate_time==0  ) ;
...
bar(mean_warp);
hold on
errorbar([1 2 3 4], mean_warp, se_warp, '.k', 'LineWidth', 1);set(gca, 'XTickLabel', bin_labels, 'FontName', 'Times New Roman'); % Set x-axis labels to bin ranges
...
bin(:,3) = deliberate;
bin(:,1) = ( ABCD_SEQ==1 & deliberate ==0 ) ;
bin(:,2) = (  DCBA_SEQ  & deliberate ==0 ) ;
bin(:,4) = bin(:,1)+bin(:,2)+bin(:,3)==0;
...
% y_bracket_12 = 12;  % Bin 1 vs Bin 2
% y_bracket_23 = 13.5;  % Bin 2 vs Bin 3
y_bracket_34 = 15.7;  % Bin 3 vs Bin 4
% y_bracket_13 = 16.5;  % Bin 1 vs Bin 3
y_bracket_14 = 12.7;  % Bin 1 vs Bin 4
