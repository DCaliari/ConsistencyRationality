...
for i = 1:length(bin_labels)
    ...
    mean_warp(i) = mean(swapsTime(bin(:,i)));
    se_warp(i) = std(swapsTime(bin(:,i))) / sqrt(length(swapsTime(bin(:,i))));
end
...
[~, p34, ~] = ttest2(swapsTime(bin(:,3)), swapsTime(bin(:,4)),'Tail','right');
[~, p14, ~] = ttest2(swapsTime(bin(:,1)), swapsTime(bin(:,4)),'Tail','left');
[~, p24, ~] = ttest2(swapsTime(bin(:,2)), swapsTime(bin(:,4)),'Tail','left');
...
for i = 1:length(bin_labels)
    ...
    bin_labels{i} = sprintf('%s (n=%d)', bin_labels{i}, bin_count);
    % Calculate mean "Response_D" for entries within this bin
    mean_warp(i) = mean(swapsRisk(bin(:,i)));
    se_warp(i) = std(swapsRisk(bin(:,i))) / sqrt(length(swapsRisk(bin(:,i))));
end
...
[~, p34, ~] = ttest2(swapsRisk(bin(:,3)), swapsRisk(bin(:,4)),'Tail','right');
[~, p14, ~] = ttest2(swapsRisk(bin(:,1)), swapsRisk(bin(:,4)),'Tail','left');
[~, p24, ~] = ttest2(swapsRisk(bin(:,2)), swapsRisk(bin(:,4)),'Tail','left');
