...
for i = 1:length(bin_labels)
    ...
    mean_warp(i) = mean(AC_time(bin(:,i)));
    se_warp(i) = std(AC_time(bin(:,i))) / sqrt(length(AC_time(bin(:,i))));
end
...
ylabel('Always Chosen violations', 'FontName', 'Times New Roman');
ylim([0 6])

% Perform t-tests and store results for p-value annotations

[~, p34, ~] = ttest2(AC_time(bin(:,3)), AC_time(bin(:,4)),'Tail','right');
[~, p14, ~] = ttest2(AC_time(bin(:,1)), AC_time(bin(:,4)),'Tail','left');
[~, p24, ~] = ttest2(AC_time(bin(:,2)), AC_time(bin(:,4)),'Tail','left');

% Define x positions for the bars
x_positions = 1:length(mean_warp);

y_bracket_34 = 5;  % Bin 3 vs Bin 4
y_bracket_14 = 3;  % Bin 1 vs Bin 4
y_bracket_24 = 4; % Bin 2 vs Bin 4
...
for i = 1:length(bin_labels)
    ...
    bin_labels{i} = sprintf('%s (n=%d)', bin_labels{i}, bin_count);
    % Calculate mean "Response_D" for entries within this bin
    mean_warp(i) = mean(AC_risk(bin(:,i)));
    se_warp(i) = std(AC_risk(bin(:,i))) / sqrt(length(AC_risk(bin(:,i))));
end
...
[~, p34, ~] = ttest2(AC_risk(bin(:,3)), AC_risk(bin(:,4)),'Tail','right');
[~, p14, ~] = ttest2(AC_risk(bin(:,1)), AC_risk(bin(:,4)),'Tail','left');
[~, p24, ~] = ttest2(AC_risk(bin(:,2)), AC_risk(bin(:,4)),'Tail','left');
