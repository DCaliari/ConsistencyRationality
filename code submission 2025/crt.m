...
for i = 1:length(bin_labels)
    ...
    % Calculate mean "Response_D" for entries within this bin
     mean_raven(i) = mean(CRT(bin(:,i)));
    se_raven(i) = std(CRT(bin(:,i))) / sqrt(length(CRT(bin(:,i))));
end
...
ylabel('CRT Scores', 'FontName', 'Times New Roman');
ylim([0 4])

% Perform t-tests and store results for p-value annotations
[h12, p12, ~] = ttest2(CRT(bin(:,1)), CRT(bin(:,2)));
[h23, p23, ~] = ttest2(CRT(bin(:,2)), CRT(bin(:,3)));
[h34, p34, ~] = ttest2(CRT(bin(:,3)), CRT(bin(:,4)),'Tail','right');
[h13, p13, ~] = ttest2(CRT(bin(:,1)), CRT(bin(:,3)));
[h14, p14, ~] = ttest2(CRT(bin(:,1)), CRT(bin(:,4)),'Tail','left');
[h24, p24, ~] = ttest2(CRT(bin(:,2)), CRT(bin(:,4)),'Tail','left');
...
y_bracket_34 = 3.5;  % Bin 3 vs Bin 4
% y_bracket_13 = 16.5;  % Bin 1 vs Bin 3
y_bracket_14 = 2.5;  % Bin 1 vs Bin 4
y_bracket_24 = 3; % Bin 2 vs Bin 4
...
plot([x_positions(3), x_positions(3), x_positions(4), x_positions(4)], [y_bracket_34, y_bracket_34+0.1, y_bracket_34+0.1, y_bracket_34], 'k', 'LineWidth', 1.5);
text(mean(x_positions(3:4)), y_bracket_34 + 0.25, sprintf('p = %.3f', p34), 'HorizontalAlignment', 'center', 'FontSize', 10, 'FontName', 'Times New Roman');
hold on
...
plot([x_positions(1), x_positions(1), x_positions(4), x_positions(4)], [y_bracket_14, y_bracket_14+0.1, y_bracket_14+0.1, y_bracket_14], 'k', 'LineWidth', 1.5);
text(mean([x_positions(1), x_positions(4)]), y_bracket_14 + 0.25, sprintf('p = %.3f', p14), 'HorizontalAlignment', 'center', 'FontSize', 10, 'FontName', 'Times New Roman');
hold on
% Sixth comparison (Bin 2 vs Bin 4)
plot([x_positions(2), x_positions(2), x_positions(4), x_positions(4)], [y_bracket_24, y_bracket_24+0.1, y_bracket_24+0.1, y_bracket_24], 'k', 'LineWidth', 1.5);
text(mean([x_positions(2), x_positions(4)]), y_bracket_24 + 0.25, sprintf('p = %.3f', p24), 'HorizontalAlignment', 'center', 'FontSize', 10, 'FontName', 'Times New Roman');
